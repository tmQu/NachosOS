...
{
    if(strcmp(threadName, "main") == 0)
        pid = 0;
    else
        pid = -1; // ProccessTable doesn't manage this thread
    priority = 0; // highest prirority
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    status = JUST_CREATED;
...
Thread::Thread(char* threadName, int id, int prio)
{
    pid = id;
    priority = prio;
    name = threadName;
...
Thread::~Thread()
{
    ...
    {
        delete name;
        //deallocate the thread name manage by the ProcssTable
    }
...
void 
Thread::ForkWithPriority(VoidFunctionPtr func, int arg)
{
    DEBUG('t', "Forking thread \"%s\" with func = 0x%x, arg = %d\n",
	  name, (int) func, arg);
    ...
    IntStatus oldLevel = interrupt->SetLevel(IntOff);
    scheduler->ReadyToRunWithPriority(this, this -> getPriority());	// ReadyToRun assumes that interrupts 
					// are disabled!
    (void) interrupt->SetLevel(oldLevel);
}    
