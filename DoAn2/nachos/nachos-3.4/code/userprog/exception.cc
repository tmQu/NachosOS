        ...
        case SyscallException:
            switch(type)
            {
				...
				case SC_Exec:
				{
                    ...
                    // Thread* thr = new Thread(fileName);
                    // int addrFileName = (int) fileName;
                    // thr->Fork(StartProcess, addrFileName);
					if(priority < 0)
					{
						printf("\nCannot execute %s, priority >= 0\n", fileName);
                        machine->WriteRegister(2, -1); 
          
                        delete fileName;
                        break; // use break to increaseProgramCounter
					}
                    int res = gPTable -> createNewProcess(fileName, priority);
      		...
      		case SC_Exit:
        	{
            	gPTable -> cleanUpCurrentProcess(); // free up the main memory

				if(currentThread -> getPid() == 0) // main thread -> call Halt()
				{
					printf("\n\nmain thread exit\n");
					interrupt -> Halt();
				}
